...
export MESH_AIOPSLAB_PROBLEM_ANCHOR_ENABLED=0
...
RD="data/results/mesh-aiopslab-payment-unreachable-${TS}"
LOG="logs/mesh-aiopslab-payment-unreachable-${TS}.log"
mkdir -p "$RD" logs
printf 'RD=%s\nLOG=%s\nMESH=%s\nSTARTED=%s\nPROBLEM=%s\n' \
  "/root/AIOpsLab/$RD" \
  "/root/AIOpsLab/$LOG" \
  ...
  > /root/AIOpsLab/.last_payment_unreachable_run.txt
